...
- Phase 5: `wa mcp connect` now maintains a real cross-command MCP connection through a supervised process; live status includes discovered tool/resource/prompt counts, connection failures are normalized, and connect/disconnect/disable/remove/configuration operations serialize on a per-server lock. Added a real subprocess CLI test using a local official-SDK STDIO fixture.
